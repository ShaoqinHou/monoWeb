...
export function EditableEntriesTable({ entries, onChange }: EditableEntriesTableProps) {
  ...
  return (
    <div>
          ...
          {showAddGroup ? (
            <div className="flex items-center gap-1">
              <input
                ...
                className="w-28 rounded border border-zinc-300 bg-white px-2 py-0.5 text-xs dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
                autoFocus
              />
              <button type="button" onClick={addGroup} className="rounded border border-zinc-300 px-1.5 py-0.5 text-xs text-zinc-600 hover:bg-zinc-100 dark:border-zinc-700 dark:text-zinc-400 dark:hover:bg-zinc-800">Add</button>
              <button type="button" onClick={() => { setShowAddGroup(false); setNewGroupName(''); }} className="px-1 text-xs text-zinc-400 hover:text-zinc-600">Cancel</button>
            </div>
          ) : (
            <>
              <button type="button" onClick={() => setShowAddGroup(true)} className="rounded border border-zinc-300 px-2 py-0.5 text-xs text-zinc-600 hover:bg-zinc-100 dark:border-zinc-700 dark:text-zinc-400 dark:hover:bg-zinc-800">+ Group</button>
              <button type="button" onClick={addSummaryEntry} className="rounded border border-zinc-300 px-2 py-0.5 text-xs text-zinc-600 hover:bg-zinc-100 dark:border-zinc-700 dark:text-zinc-400 dark:hover:bg-zinc-800">+ Summary</button>
            </>
          )}
        </div>
      </div>
      ...
      {entries.length === 0 ? (
        <p className="text-xs text-zinc-400">No entries</p>
      ) : (
                ...
                {summaryEntries.map(({ entry, globalIndex }) => (
                  <div key={globalIndex} className="flex items-center gap-2">
                    <input
                      ...
                      placeholder="Label"
                      className="flex-1 rounded border border-zinc-300 bg-white px-2 py-1 text-sm font-medium dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
                    />
                    <select
                      value={entry.entry_type ?? 'subtotal'}
                      onChange={e => updateEntry(globalIndex, 'entry_type', e.target.value)}
                      className="w-20 rounded border border-zinc-300 bg-white px-1 py-1 text-xs dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
                    >
                    ...
                    <input
                      ...
                      placeholder="Amount"
                      className="w-24 rounded border border-zinc-300 bg-white px-2 py-1 text-right text-sm font-medium tabular-nums dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
                    />
...
/** Cell input style — Excel-like border for easy copy-paste selection */
const cellClass = 'border border-zinc-200 bg-white px-2 py-1 text-sm dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100';
const cellInputClass = `${cellClass} w-full outline-none focus:ring-1 focus:ring-blue-300`;
...
function GroupSection({ group, onUpdate, onUpdateAttr, onRemove, onAdd, onRename }: {
  ...
  return (
    <div className="rounded-lg border border-zinc-100 dark:border-zinc-800">
      {/* Group header */}
      <div className="flex items-center justify-between rounded-t-lg bg-zinc-50 px-3 py-1.5 dark:bg-zinc-900/50">
        {editingName ? (
          <input
            value={name}
            ...
            className="rounded border border-zinc-300 bg-white px-1.5 py-0.5 text-xs font-semibold uppercase dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
            autoFocus
          />
        ) : (
          <button
            onClick={() => setEditingName(true)}
            className="text-xs font-semibold uppercase tracking-wider text-zinc-400 hover:text-zinc-600 dark:text-zinc-500 dark:hover:text-zinc-300"
            title="Click to rename group"
          >
            {titleCase(group.type)}
          </button>
        )}
        <button type="button" onClick={onAdd} className="rounded px-1.5 py-0.5 text-xs text-zinc-400 hover:text-zinc-600 dark:text-zinc-500 dark:hover:text-zinc-300">
          + Add
        </button>
      </div>

      {/* Table with attrs columns — Excel-like cells */}
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm" style={{ borderCollapse: 'collapse' }}>
          <thead>
            <tr className="bg-zinc-50 text-xs text-zinc-500 dark:bg-zinc-900/30">
              <th className="border border-zinc-200 px-2 py-1 font-medium dark:border-zinc-700">Entry</th>
              <th className="border border-zinc-200 px-2 py-1 font-medium text-right dark:border-zinc-700 w-24">Amount</th>
              {group.columns.map(col => (
                <th key={col.key} className="border border-zinc-200 px-2 py-1 font-medium dark:border-zinc-700">{col.label}</th>
              ))}
              <th className="border border-zinc-200 px-1 py-1 dark:border-zinc-700 w-8" />
            </tr>
          </thead>
          <tbody>
            {group.entries.map(({ entry, globalIndex }) => {
              const attrs = entry.attrs ?? {};
              return (
                <tr key={globalIndex} className="hover:bg-blue-50/30 dark:hover:bg-blue-950/20">
                  <td className="border border-zinc-200 p-0 dark:border-zinc-700">
                    <input
                      value={entry.label}
                  ...
                  {group.columns.map(col => (
                    <td key={col.key} className="border border-zinc-200 p-0 dark:border-zinc-700">
                      <input
                  ...
                  <td className="border border-zinc-200 px-1 py-1 text-center dark:border-zinc-700">
