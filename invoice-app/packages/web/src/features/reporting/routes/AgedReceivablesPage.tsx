...
const AGEING_BY_OPTIONS = ['Due Date', 'Invoice Date', 'Created Date'];
const GROUPING_OPTIONS = ['None', 'Contact', 'Invoice Status', 'Currency'];

export function AgedReceivablesPage() {
  const { data: report, isLoading } = useAgedReceivables();
  ...
  const handleExport = useCallback(() => {
    ...
    downloadCsv(csv, 'aged-receivables.csv');
  }, [report]);
  ...
  return (
    ...
    >
      <ReportHeader title="Aged Receivables" subtitle={subtitle} onExport={handleExport}>
                  ...
                  {report.buckets.map((bucket) => (
                    <TableRow key={bucket.label} data-testid={`bucket-row-${bucket.label}`}>
                      ...
                      <TableCell className="text-right">
                        <Link
                          to={`/invoices?status=outstanding&aging=${encodeURIComponent(bucket.label)}` as string}
            ...
            <CardContent>
              ...
              <div className="flex flex-col gap-2">
                <Link
                  ...
                  data-testid="related-aged-payables"
                >
